...
# * Time taken: 32 minutes + additional for comments
...
def reverseRecursive(head):
    ...
    # case for no head or one element
    if not head or not head.next:
        return head

    # we update head to the last element recursively
    updated_head = reverseRecursive(head.next)

    # with the updated head value, we go on reversing the nodes from the second last node
    head.next.next = head
    head.next = None

    # We return the updated head in each recursive call
    return updated_head
